<template>
  <van-cell-group class="base-form">
    <van-field v-for="(item, index) in formList" :key="index" v-model="form[item.value]"  :placeholder="item.remind" :label="item.label" :type="item.type">
      <img v-if="item.icon"  :width="iconWidth"  :height="iconHeight" slot="left-icon" :src="item.icon" alt=""/>
      <!--      todo 发送短信验证码-->
      <van-button v-if="item.mySolt === 'sendCode'" slot="button"
        size="small"
        type="primary"
        @click="mySlotClick"
        >发送验证码</van-button>
...
export default {
  ...
  methods: {
    ...
    onmarked(type, result) {
     ...
     /*  this.form['equiNo'] = 7878787
      console.log(" this.form['equiNo'] 组件", this.form)
      console.log("dddddddddddd",text + result); */
