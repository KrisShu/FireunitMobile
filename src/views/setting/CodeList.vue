<template>
  <div>
    <base-nav title="设施编号列表"  @onClickRight="$router.push('./BingNumber/0')">
      <van-icon name="plus" slot="rightText"></van-icon>
    </base-nav>

    <base-form :form="form" :form-list="formList" @codeInfo="setCode"></base-form>
    <base-list
      @onLoad="getList"
      @refresh="getList"
  ...
  methods: {
    ...
    getList(success) {
      ...
      this.$axios
        ...
        .then(res => {
          if (res.success) {
            console.log("获取设施编号列表",res.result.equipmentNoList)
